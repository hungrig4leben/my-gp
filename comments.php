 ...
 * handled by a callback to gp_comment().
 *
 * @package gp
 * @since gp 1.0
 */
...
?>
	...
	<?php if ( have_comments() ) : ?>
		<h2 class="widget-title comments-title">
			...
			<?php
				printf(
						_n( '1 comment', '%1$s comments', get_comments_number(), 'gp' ),
		...
		<nav role="navigation" id="comment-nav-below" class="comment-navigation">
			<h1 class="assistive-text"><?php _e( 'Comment navigation', 'gp' ); ?></h1>
			<?php
				previous_comments_link( __( 'Older Comments', 'gp' ) );
				next_comments_link( __( 'Newer Comments', 'gp' ) );
			?>
		</nav>
	...
	?>
		<p class="nocomments"><?php _e( 'Comments are closed.', 'gp' ); ?></p>
	<?php endif; ?>
