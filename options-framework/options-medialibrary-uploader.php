...
if ( ! function_exists( 'optionsframework_image_select' ) ) {

	function optionsframework_image_select($id, $value) {
		...
		if ( is_numeric( $value ) ) {
			...
			if ( isset( $image_data[0] ) ) {
				$image = '<img src="' . esc_url( $image_data[0] ) . '" alt="" />';
			}
		} else if ( !empty( $value ) ) {
			// Legacy case.
			// $value contains URL of the image
			$class = ' image-selected';
			$image = '<img src="' . esc_url( $value ) . '" alt="" />';
		}

		$output .= '<div class="image-select-control' . $class . '">';
		$output .= '<a href="#" class="image-container"><div class="centered">' . $image . '</div></a>';
		$output .= '<div class="no-image">' . __( 'No image', 'gp' ) . '</div>';
		$output .= '<input id="' . $id . '" class="selected-file-id" type="hidden" name="'.$name.'" value="' . $value . '" />' . "\n";
		$output .= '<input class="image-select-button button" type="button" value="' . __( 'Upload', 'optionsframeworktheme' ) . '" />' . "\n";
		$output .= '<input class="image-change-button button" type="button" value="' . __( 'Change', 'optionsframeworktheme' ) . '" />' . "\n";
		$output .= '<a href="#" class="remove-image button">' . __( 'Remove', 'optionsframeworktheme' ) . '</a>';
		$output .= '</div>';
