...
class PortfolioMedia extends gpPage {
    ...
    function get_alt() {
        ...
        $alt = trim( strip_tags( $this->meta_description ) );

        if ( ! $alt ) {
            if ( $this->is_image() ) {
                $data = $this->get_image_data( 'gp-max' );
                if ( $data ) {
                    $alt = $data['title'];
                }
            }
    ...
    // Returns image data array, or empty array on failure.
    function get_thumbnail( $size = 'gp-thumbnail' ) {

        if ( $this->is_image() ) {
            return $this->get_image_data( $size );
        } else {
            return $this->get_video_thumbnail( 'gp-thumbnail' );
        }

    }

    function get_video_thumbnail( $size = 'gp-thumbnail' ) {
    ...
    function initialize_image_size( $size ) {
        ...
        if ( $image ) {
            ...
            $post = get_post( $this->meta_data['attachment_id'] );
