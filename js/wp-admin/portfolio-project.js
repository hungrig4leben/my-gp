...
;(function ($, window) {
    ...
    var post_id = window.gp_post_id || 0;

    $(function () {

        var $btnAdd = $('#gp-add-project-info'),
            $container = $('.gp-project-information tbody');

        $btnAdd.click(function (e) {

            e.preventDefault();

            var $title = $( 'input[name="gp_project_info_add_title"]' ),
                $content = $( 'textarea[name="gp_project_info_add_content"]' ),
                $newElement = $( '<tr class="gp-project" />' );

            $( '<input type="text" name="gp_project_info_title[]" />' ).val( $title.val() )
                                                                           .appendTo( $newElement )
                                                                           .wrap( '<td />' );

            $( '<textarea name="gp_project_info_content[]" />' ).val( $content.val() )
                                                                    .appendTo( $newElement )
                                                                    .wrap( '<td />' );
    ...
    var ProjectMediaModel = Backbone.Model.extend({
        ...
        url: function () {
            ...
            return ajaxurl + '?action=gp_project_media_save&post_id=' + post_id + ( this.id ? '&id=' + this.id : '' ) + '&nonce=' + portfolio_media_nonce;
        },
    ...
    var ProjectMediaList = Backbone.Collection.extend({
        ...
        url: ajaxurl + '?action=gp_project_media&post_id=' + post_id
    ...
    var ProjectMediaApp = Backbone.View.extend({
        ...
        events: {
            'click #gp-add-image':      'addImageEvent',
            'click #gp-add-video':      'addVideoEvent'
        },
