(function ( $, window, isUndefined ) {
    ...
    $(function () {
         ...
         */
        $slideTable.find('.gp-save-slide').live( 'click', function () {
        ...
        $('#gp-slider-options-form').submit(function () {

            var $form = $(this),
                $submit = $form.find('[type="submit"]');

            if ($form.is('.form-status-saving')) {
                return false;
            }

            $form.removeClass('form-status-saved').addClass('form-status-saving');
            ...
            $.ajax({
                ...
                type: 'post',
                success: function () {
                    $form.removeClass('form-status-saving').addClass('form-status-saved');
                    $submit.removeAttr('disabled');
                    setTimeout(function () {
                        $form.removeClass('form-status-saved');
                    }, 2000);
                },
                error: function () {
                    $form.removeClass('form-status-saving').addClass('form-status-error');
                    $submit.removeAttr('disabled');
                    setTimeout(function () {
                        $form.removeClass('form-status-error');
                    }, 2000);
                }
            });
            ...
            if ('gp_slider_slideshow' === $t.attr('name')) {

                value = $t.is(':checked') ? 1 : 0;

            }

            if ('gp_slider_slideshow_interval' === $t.attr('name')) {
            ...
            if ('gp_slider_slideshow_animation' === $t.attr('name')) {
